...
def TABLEHEAD(n):
    ...
    lines.append(
        "Name & EID & " + " & ".join(str(i + 1) for i in range(n)) + r" \\\hline"
    )
    return "\n".join(lines)
...
class Roster(cli.Application):
    ...
    number = cli.SwitchAttr(
        ["-n", "--number"],
        cli.Range(1, 12),
        default=11,
        help="Number of columns in gradesheet",
    )
    _schedule = None
    ...
    def main(self, filename):
        ...
        semester = (
            "Spring" if date.month < 5 else "Summer" if date.month < 8 else "Fall"
        )
        ...
        with open(str(outname), "w") as f:
            ...
            for val in sorted(set(students["Unique"])):
                ...
                class_header = r"{} \quad {} {} -- {} students".format(
                    int(val), semester, date.year, len(one_class.index)
                )
                if self._schedule:
            ...
            {\bfseries\large""",
                    class_header,
                    (ROSTTABLEHEAD if self.roster else TABLEHEAD(self.number)),
                )
        ...
        pdflatex[outname] & FG
        delete(outname, outname.with_suffix(".log"), outname.with_suffix(".aux"))
