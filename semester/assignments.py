...
from plumbum import CommandNotFound, cli, local
...
def process(filename, outext, ans, prepend=None):
    ...
    ansheading = f"#{name} Rubric. Points: {pts}\n\n"
    txt = txt.replace("(1)", "**(1 point)**")
    txt = ansheading + RE_POINTS.sub(r"**(\g<1> points)**", txt)

    (
        pandoc[
            "-V", "geometry:margin=1in", "-f", INPUTFORM, "-o", str(outname), out_opts
        ]
...
Processes all *.{} if no file given.""".format(
        INPUTEXT
    )
    ...
    def main(self, *files):
        ...
        items = (
            list(map(cli.ExistingFile, files))
            if files
            else local.cwd // ("*." + INPUTEXT)
        )
        try:
