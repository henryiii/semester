...
def locator(driver):
    return expected_conditions.presence_of_element_located(
        (By.CLASS_NAME, "question_input"),
    )(driver) or expected_conditions.presence_of_element_located((By.ID, NO_SUB_ID),)(
        driver
    )
...
def main():
    ...
    while True:
        ...
        while (
            len(driver.find_elements_by_xpath(blank_path))
            or len(driver.find_elements_by_id(NO_SUB_ID)) > 0
        ):
