...
class Time:
    ...
    def make_day(self):
        ...
        return f"{WEEK[self.day]:<9} {hour:2}:00 {ampm}"

    def make_day_simple(self):
        return f"{SWEEK[self.day]}{self.hour}"

    def make_day_range(self):
        return f"{SWEEK[self.day]} {self.hour}00 to {self.hour + 2}00"
...
class TA:
    ...
    def make_web(self, times):
        ...
        sections = "<br/>".join(
            a + " " + Time(times.ix[a]).make_day() for a in self.sections
        )
    ...
    def make_tex(self, times):
        ...
        sections = ", ".join(
            a + " " + Time(times.ix[a]).make_day_simple() for a in self.sections
        )
        return (self.name, sections, self.email, self.phone)

    def make_email(self, lab, labtime):
        return TABLE.format(
            section=lab, day=labtime, eid=self.eid, name=self.name.replace('\\"u', "ue")
        )
...
class AllInfo:
    ...
    def init_template(self, forced_recreate=False):
        if forced_recreate or self.tmpl is None:
            ...
            added_red = [
                [item.date, item.name]
                if "no lab" not in item.name.lower()
                else [r"\emph{" + item.date + "}", r"\emph{" + item.name + "}"]
                for item in schedules
            ]
            self.tmpl["firstlab"] = (
                schedules[0].date
                if "no lab" not in schedules[0].name.lower()
                else schedules[1].date
            )
            ...
            self.tmpl["tatable"] = pytem.make_latex_table(
                ["Name", "Sections", "Email", "Phone"], talist
            )
            ...
            for section in sorted(self.lablist.index):
                ...
                for ta in tas:
                    if section in ta.sections:
                        f.write(
                            ta.make_web_section(
                                section, Time(self.lablist.ix[section]).make_day()
                            )
                        )
                        section_found = True
                        break
                if not section_found:
                    f.write(
                        TA.notavailable().make_web_section(
                            section, Time(self.lablist.ix[section]).make_day()
                        )
                    )
    ...
    def write_email_talist(self):
        ...
        with open(OUT_TALIST_EMAIL, "w") as f:
            f.write(
                "This is the assignment of the TAs in the lab sections for 102M:\n\n"
            )
            f.write(TABLEHEADER)
            for section in sorted(self.lablist.index):
                section_found = False
    ...
    def make_latex_syllabus(self, taeid, filename=PYTEM_STY):
        ...
        return pytem.compileLaTeX(
            SOURCE,
            FINAL,
            "syllabus",
            "Syllabus_102M_" + ta.name.replace(" ", "_").replace('\\"u', "ue"),
        )
...
def copy_web_files():
    ...
    for file_o, file_s in (TAFILE, CFILE):
        ...
        with open(file_s) as f:
            add_table = (
                '<table border="1" cellpadding="2" cellspacing="2" class="content_table">'
                + f.read()
                + r"</table>"
            )
