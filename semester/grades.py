...
from tkinter_tkfiledialog import askopenfilename, askopenfilenames, asksaveasfile


class Cutoffs:
    def __init__(self, students):
        self.gpacuts = np.array([60, 62, 68, 70, 72, 78, 80, 82, 88, 90, 92.0]) + 0.5
        self.gpanames = np.array(
            ["F", "D-", "D", "D+", "C-", "C", "C+", "B-", "B", "B+", "A-", "A"]
        )
        self.gpavalues = np.array(
            [
                0,
                1 - 1 / 3,
                1,
    ...
    def listhist(self):
        return [
            f"{nm}:{hist}"
            for hist, nm in zip(self.hist()[::-1], self.gpanames[::-1])
            if hist > 0
        ]
    ...
    def listcuts(self):
        return [
            f"{self.gpacuts[-n - 1]}:{self.gpanames[-n - 1]}"
            for n in range(len(self.gpacuts))
        ]

    def predict_cuts(self):
        cuts = (predict_cuts(self.student_list["Score"]) - 0.5).round() + 0.5
        return np.fmax(np.fmin(cuts, 99.5), 9.5)

    def itcuts(self):
        return [
            (self.gpanames[-n - 1], self.gpacuts[-n - 1])
            for n in range(len(self.gpacuts))
        ]

    def __repr__(self):
        return f"{self.num_students} students, Average GPA:{self.check():.2f}"

    def __str__(self):
        return repr(self)

    def gpastring(self):
        return f"GPA:{self.check():.2f}"
    ...
    def plot(self):
        ...
        fig.subplots_adjust(right=0.76, left=0.05)
        self.fig = fig
        ...
        # set lines
        self.listlines = [plt.axhline(a, axes=ax, alpha=0.3) for a in self.gpacuts]

        gpacutsforav = np.append(self.min, np.append(self.gpacuts, self.max))
        self.listlabels = [
            ax.annotate(
                let,
                (self.num_students - 0.7, (gpacutsforav[n] + gpacutsforav[n + 1]) / 2),
        ...
        # Main plots
        self.bar = ax.bar(
            X + 0.1, self.student_list["Score"].round(), alpha=0.2, color="b"
        )
        ax.scatter(X + 0.5, self.student_list["Score"], color="r")
        ...
        ax.annotate(
            ...
            textcoords=ofsetsm,
            annotation_clip=False,
            arrowprops=dict(arrowstyle="->", color="g", alpha=0.8),
        )
        ax.annotate(
            "",
            xy=(0, totalmean - totalstd),
        ...
        # Title
        ax.set_title(
            "Mean: {:.2f}, Median: {:.2f}, Stddev: {:.2f}".format(
                totalmean, np.median(self.student_list["Score"]), totalstd
            )
        )
        ...
        self.sideplot = self.histax.bar(
            np.arange(len(self.gpanames)) + 0.1, self.hist()
        )
        self.histax.set_xbound(lower=0, upper=len(self.gpanames))
        self.histax.set_xticks(np.arange(1, len(self.gpanames) + 1) - 0.5)
        self.histax.set_xticklabels(self.gpanames)
        self.histax.set_ybound(upper=max(self.hist()) + 0.5)
        self._colorbars()
    ...
    def _savegrades(self, event=None):
        ...
        root = Tk()
        root.withdraw()
        csvfile = asksaveasfile(
            mode="w", defaultextension=".csv", title="Choose a file to save grades to"
        )
        root.destroy()
        if csvfile:
    ...
    def _savecuts(self, event=None):
        ...
        root.withdraw()
        csvfile = asksaveasfile(
            mode="w", defaultextension=".csv", title="Choose a file to save cuts to"
        )
        root.destroy()
        if csvfile:
    ...
    def _saveimport(self, event=None):
        ...
        csvfile = asksaveasfile(
            ...
            title="Choose a file to save registrar import to",
        )
        root.destroy()
        if csvfile:
            sorted_students = self.student_list.sort_values(by=["Last", "First"])
            ...
            finalgrades["Name"] = (
                sorted_students["Last"] + ", " + sorted_students["First"]
            )
    ...
    def _loadcuts(self, event=None):
        ...
        if platform.system() != "Windows":
            filetypes.append(
                filetypes.pop(0)
            )  # Reorder so the all gradefiles is selected first

        root = Tk()
        root.withdraw()
        csvfile = askopenfilename(
            filetypes=filetypes, title="Choose a file to load cuts from"
        )
        root.destroy()
    ...
    def _togglelabels(self, event=None):
        ...
        for n, label in enumerate(self.labelpoints):
            ...
            if self.label_state != 0:
                label.set_text(
                    self.student_list.iloc[n]["First"]
                    + " "
                    + self.student_list.iloc[n]["Last"][0]
                    + "."
                    if self.label_state == 1
                    else self.whatgrade(self.student_list.iloc[n]["Score"])
                )
    ...
    def _colorbars(self):
        for n, bar in enumerate(self.bar):
            if self.whatgrade(self.student_list.iloc[n]["Score"]) == self.whatgrade(
                self.student_list["Score"].round()[n]
            ):
                bar.set_color("b")
            else:
                bar.set_color("r")
    ...
    def _change(self, linenum, changeto):
        if (
            self.gpacutsfull[linenum] < changeto < self.gpacutsfull[linenum + 2]
            and self.gpacutsfull[linenum + 1] != changeto
        ):
            ...
            gpacutsforav = np.append(self.min, np.append(self.gpacuts, self.max))
            self.listlabels[linenum].xyann = (
                self.num_students - 0.7,
                (gpacutsforav[linenum] + gpacutsforav[linenum + 1]) / 2,
            )
            self.listlabels[linenum + 1].xyann = (
                self.num_students - 0.7,
                (gpacutsforav[linenum + 1] + gpacutsforav[linenum + 2]) / 2,
            )
            self.lowlab.set_color("g" if 2.95 < self.check() < 3.05 else "r")
            self._colorbars()
...
class LineChanger:
    ...
    def on_motion(self, event):
        if self.pressed and event.inaxes == self.ax:
            newval = np.round(event.ydata - 0.5) + 0.5 if self.stepping else event.ydata
    ...
    def connect(self, figure, ax):
        ...
        self.cidrelease = figure.canvas.mpl_connect(
            "button_release_event", self.on_release
        )
        self.cidmotion = figure.canvas.mpl_connect(
...
def readcsv(name):
    ...
    students = pd.read_csv(
        name, index_col=3, skiprows=[1]
    )  # Making some assumptions about the Canvas format
    students["Student"]
    ...
    student_list["Class"] = classes.apply(int)
    student_list["Grades"] = ""
...
def predict_cuts(arr):
    ...
    std = max(std, 3)
    mini = max(mini, 1)
    ab, bc, cd, df = av + std * 0.5, av - std * 0.5, av - std * 1.5, av - std * 2.5
    return np.array(
        [
            df,
            df + mini,
...
def ask_filenames():
    ...
    filenames = askopenfilenames(
        filetypes=filetypes, title="Select a file or files to open"
    )
    root.destroy()

    # Fix for broken Windows Tk
    if isinstance(filenames, str):
        filenames_extra = re.findall(r"\{(.*?)\}", filenames)
        filenames = re.sub(r"\{(.*?)\}", "", filenames)
